...
const Input: React.FC<InputProps> = ({ label, placeholder, name, id, type, value, onChange, error }) => {
  ...
  const needIcon = type === 'password'

  const toggleIconState = () => {
    setIconState(!iconState);
  };

  const visiableEye = <svg className={`constIcon  ${error ? 'icon-error': ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#000000" fill="none">
  ...
  const hiddenEye = <svg className={`constIcon  ${error ? 'icon-error': ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#000000" fill="none">
  ...
  return (
    ...
    <div className={`input-wrapper ${error ? 'input-error' : ''}`}>
      ...
      <input
        type={needIcon && iconState ? 'text' : type}
      ...
      {needIcon && (
        <span className='icon' onClick={toggleIconState}>
          {iconState ? hiddenEye : visiableEye}
        </span>
      )}
    </div>
  );
